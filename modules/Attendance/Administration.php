...
if(isset($_REQUEST['student_id']) && $_REQUEST['student_id']!='new')
{
	...
	$schedule_RET = DBGet(DBQuery("SELECT
										s.STUDENT_ID,c.TITLE ||' - '||s.COURSE_WEIGHT AS COURSE,cp.PERIOD_ID,cp.COURSE_PERIOD_ID,p.TITLE AS PERIOD_TITLE,
	...
	DrawHeaderHome('<A HREF='.$_SESSION['List_PHP_SELF'].'&bottom_back=true target=body  style="text-decoration:none"><strong>Back to Student List</strong></A>');
	ListOutput($schedule_RET,$columns,LANG_COURSE,LANG_COURSES);
	echo '</FORM>';
}
else
{
	...
	DrawHeader(PrepareDate($date,'_date',false,array('submit'=>true)),'<TABLE><TR><TD>'.$current_student_link.button('add','',"# onclick='javascript:addHTML(\"".str_replace('"','\"',_makeCodeSearch())."\",\"code_pulldowns\"); return false;'").'</TD><TD><DIV id=code_pulldowns>'.$code_pulldowns.'</DIV></TD></TR></TABLE>');
	...
	{
		echo '<TABLE width=100% border=0 cellpadding=0 cellspacing=0 bgcolor=#FFFFFF style="border:1;border-style: none none solid none;"><TR><TD>';
...
{	global $THIS_RET,$contacts_RET;
	...
	$tipmessage = 'This student has no contact information.';
		...
		#$tipmessage = LANG_THIS_STUDENT_HAS_NO_CONTACT_INFORMATION_;
	#return button('phone','','# onMouseOver=\'stm(["'.LANG_CONTACT_INFORMATION.'","'.str_replace("'",'&#39;',$tipmessage).'"],["white","#333366","","","",,"black","#e8e8ff","","","",,,,2,"#333366",2,,,,,"",,,,]);\' onMouseOut=\'htm()\'');
}
...
{	global $codes_RET,$code_search_selected;
	...
	$return = '<SELECT name=codes[]><OPTION value="">'.LANG_ALL.'</OPTION><OPTION value="A"'.(($value=='A')?' SELECTED':'').'>NP</OPTION>';
	if(count($codes_RET))
	{
		foreach($codes_RET as $code)
		{
