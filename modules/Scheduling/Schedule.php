...
else
{
	...
	if($min_date[1]['MIN_DATE'] && DBDate()<$min_date[1]['MIN_DATE'])
	{
...
if($_REQUEST['schedule'] && ($_POST['schedule'] || $_REQUEST['ajax']))
{
	...
	foreach($start_dates as $start_date=>$columns)
	{
		...
		DBQuery($sql);
		//Commented as it was affecting the filled seats number on hitting the save button
...
if(UserStudentID() && $_REQUEST['modfunc']!='choose_course' )
{
	...
	DrawHeaderHome(PrepareDateSchedule($date,'_date',false,array('submit'=>true)).' <INPUT type=checkbox name=include_inactive value=Y'.($_REQUEST['include_inactive']=='Y'?" CHECKED onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=\";'":" onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=Y\";'").'>Include Inactive Courses',SubmitButton('Save','','class=btn_medium onclick=\'formload_ajax("modify");\''));
	//DrawHeader(ProgramLinkforExport('Scheduling/PrintSchedules.php','Print Schedule','&modfunc=save&st_arr[]='.UserStudentID().'&_CENTRE_PDF=true target=_blank'));
	/*
