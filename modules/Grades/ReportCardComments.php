		...
		if($_REQUEST['tab_id']!='')
		{
			foreach($_REQUEST['values'] as $id=>$columns)
			{
				...
				else
				{
					...
					$fields = 'ID,SCHOOL_ID,SYEAR,COURSE_ID,';
					$values = db_seq_nextval('REPORT_CARD_COMMENTS_SEQ').',\''.UserSchool().'\',\''.UserSyear().'\','.($_REQUEST['tab_id']!='new'?"'$_REQUEST[tab_id]'":'NULL').',';
...
/*	
	if($_REQUEST['modfunc']!='remove')
	{
		...
		frmvalidator.addValidation("values[new][SORT_ORDER]","req","Please enter ID");
		frmvalidator.addValidation("values[new][SORT_ORDER]","num", "ID allows only numeric value");
		
		frmvalidator.addValidation("values[new][TITLE]","req","Please enter the Comment");
