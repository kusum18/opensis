...
if($course_period_id)
$course_RET = DBGet(DBQuery("SELECT cp.COURSE_ID,c.TITLE as COURSE_NAME, cp.TITLE, cp.GRADE_SCALE_ID,credits AS CREDITS  FROM COURSE_PERIODS cp, COURSES c WHERE cp.COURSE_ID = c.COURSE_ID AND cp.COURSE_PERIOD_ID='".$course_period_id."'"));
if(!$course_RET[1]['GRADE_SCALE_ID'])                                  
	ErrorMessage(array('You cannot enter grades for this period.'),'error');
...
if($_REQUEST['modfunc']=='gradebook')
{
	...
	include 'functions/_makeLetterGrade.fnc.php';
...
if($_REQUEST['modfunc']=='grades')
{
	if($_REQUEST['prev_mp'])
	{
	include 'functions/_makePercentGrade.fnc.php';
	...
	foreach($_REQUEST['values'] as $student_id=>$columns)
	{
		...
		if($current_RET[$student_id])
		{
			if(isset($columns['grade']) && $columns['grade']!=$current_RET[$student_id][1]['REPORT_CARD_GRADE_ID'])
			{
                ...
                $rcg_id = _makeLetterGrade($percent/100,$course_period_id,0,'ID');
                $sql .= ",GRADE_LETTER='".$grades_RET[$rcg_id][1]['TITLE']."'";
                $sql .= ",WEIGHTED_GP='".$grades_RET[$rcg_id][1]['WEIGHTED_GP']."'";
                $sql .= ",UNWEIGHTED_GP='".$grades_RET[$rcg_id][1]['UNWEIGHTED_GP']."'";
                $sql .= ",COURSE_TITLE='".$course_RET[1]['COURSE_NAME']."'";
                $sql .= ",GP_SCALE='".$grades_RET[$rcg_id][1]['GP_SCALE']."'";
                $sql .= ",CREDIT_ATTEMPTED='".$course_RET[1]['CREDITS']."'";
                $sql .= ",CREDIT_EARNED='".($grades_RET[$grade][1]['WEIGHTED_GP']>0?$course_RET[1]['CREDITS']:0)."'";
			}
		...
		elseif($columns['grade'] || $columns['percent']!='' || $columns['comment'])
		{
			...
			elseif($columns['percent']!='')
			{
 			...
 			$sql = "INSERT INTO STUDENT_REPORT_CARD_GRADES (SYEAR,SCHOOL_ID,STUDENT_ID,COURSE_PERIOD_ID,MARKING_PERIOD_ID,REPORT_CARD_GRADE_ID,GRADE_PERCENT,
                    ...
                    '".$columns['comment']."','".$grades_RET[$grade][1]['TITLE']."','".$grades_RET[$grade][1]['WEIGHTED_GP']."','".$grades_RET[$grade][1]['UNWEIGHTED_GP']."','".$course_RET[1]['COURSE_NAME']."','".$grades_RET[$grade][1]['GP_SCALE']."','".$course_RET[1]['CREDITS']."','".($grades_RET[$grade][1]['WEIGHTED_GP']>0?$course_RET[1]['CREDITS']:0)."')";
		}
...
if(!$_REQUEST['_CENTRE_PDF'])
{
	...
	DrawHeader(($current_completed?'<FONT COLOR=green>These grades are complete</FONT>':'<FONT COLOR=red>Grade reporting is not open yet for this marking period</FONT>').(AllowEdit()?' | <FONT COLOR=green>You can edit these grades</FONT>':' | <FONT COLOR=red>Grade reporting begins on this date</FONT>'));
	...
	if(AllowEdit())
	{
		...
		{
			$gb_header .= " | <A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=gradebook&mp=$_REQUEST[mp]&use_percents=false>Get Gradebook Grades</A>";
		...
		$gb_header .= " | <A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=clearall&mp=$_REQUEST[mp]&use_percents=$_REQUEST[use_percents]>Clear All</A>";
	}
...
{	global $THIS_RET,$current_RET,$grades_RET,$student_count,$tabindex;
