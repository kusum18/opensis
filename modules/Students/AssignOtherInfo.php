...
if($_REQUEST['modfunc']=='save')
{
	...
	if(count($_REQUEST['values']) && count($_REQUEST['student']))
	{
		...
		BackPrompt('You must choose at least one field and one student');
}
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		if(count($fields_RET['text']))
		{
			foreach($fields_RET['text'] as $field)
				echo '<TR><TD class=lable>'.$field['TITLE'].'</TD><TD>'._makeTextInput('CUSTOM_'.$field['ID']).'</TD></TR>';
		}
	...
	if($_REQUEST['search_modfunc']=='list')
		echo "<BR><CENTER>".SubmitButton('Save','','class=btn_medium')."</CENTER></FORM>";
}
