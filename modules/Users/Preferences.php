...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['tab']=='display_options')
	{
		...
		if($handle = opendir($CentrePath.'themes/'))
		{
		...
		#$colors = array('#e8eff1');
		#echo '<TR><TD align=right>Titles Color</TD><TD><TABLE><TR>';
		#foreach($colors as $color)
	...
	if($_REQUEST['tab']=='password')
	{
		...
		echo '<table width=100% cellspacing=6 cellpadding2><tr><td align=center><TABLE><TR><TD align=right>Current Password</TD><TD><INPUT type=password class=cell_floating name=values[current]></TD></TR>
						<TR><TD align=right>New Password</TD><TD><INPUT type=password class=cell_floating name=values[verify]></TD></TR>
						<TR><TD align=right>Verify New Password</TD><TD><INPUT type=password class=cell_floating name=values[new]></TD></TR></TABLE></td></tr></table>';
	}
