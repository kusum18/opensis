...
if($_REQUEST['modfunc']=='SendChartData' || $_REQUEST['chart_type']=='list')
{
	...
	if($category_RET[1]['DATA_TYPE']=='numeric')
		...
		$width = 20*count($category_RET[1]['SELECT_OPTIONS']) < 400 ? 400 : (20*count($category_RET[1]['SELECT_OPTIONS']));
...
//		$sql .= " GROUP BY CATEGORY_".$_REQUEST['category_id'];
		...
		$chart['chart_data'][1][] = $totals_RET['Y'][1]['COUNT'];
		$chart['chart_data'][0][] = _('No');
		$chart['chart_data'][1][] = $totals_RET['N'][1]['COUNT'];
	}
...
if(!$_REQUEST['modfunc'])
{
	...
	if($category_RET[1]['DATA_TYPE']=='numeric')
		...
		$width = 20*count($category_RET[1]['SELECT_OPTIONS']) < 400 ? 600 : 20*count($category_RET[1]['SELECT_OPTIONS'])+200;
	if($_REQUEST['chart_type']=='column')
		$height = 650;
	elseif($_REQUEST['chart_type']=='3d pie')
	{
