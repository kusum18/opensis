...
$users_columns = array('ELLIGIBLE'=>'Elligible','DAYS_POSSIBLE'=>'Days<br>Possible','TOTAL_ELLIGIBLE'=>'Total<br>Elligible','PARTICIPATED'=>'Participated');
...
if($_REQUEST['type_select']=='sales')
{
	...
	foreach($users as $user=>$discounts)
	{
		...
		$TMP_types[] = array('TYPE'=>'<b>'.$user.'</b>','DISCOUNT'=>'<b>Totals</b>','ELLIGIBLE'=>'<b>'.number_format($users_totals['']['ELLIGIBLE'],1).'</b>','DAYS_POSSIBLE'=>'<b>'.number_format($users_totals[$user]['DAYS']/$users_totals[$user]['ELLIGIBLE'],1).'</b>','TOTAL_ELLIGIBLE'=>'<b>'.$users_totals[$user]['DAYS'].'</b>','PARTICIPATED'=>'<b>'.$users_totals[$user]['PARTICIPATED'].'</b>','TOTAL'=>'<b>'.number_format($total,2).'</b>') + array_map('bold_format',$types_totals[$user]);
		unset($TMP_types[0]);
		$LO_types[] = $TMP_types;
	}
	...
	$LO_types[] = array(array('TYPE'=>'<b>Totals</b>','ELLIGIBLE'=>'<b>'.number_format($users_totals['']['ELLIGIBLE'],1).'</b>','DAYS_POSSIBLE'=>'<b>'.number_format($users_totals['']['DAYS']/$users_totals['']['ELLIGIBLE'],1).'</b>','TOTAL_ELLIGIBLE'=>'<b>'.$users_totals['']['DAYS'].'</b>','PARTICIPATED'=>'<b>'.$users_totals['']['PARTICIPATED'].'</b>','TOTAL'=>'<b>'.number_format($total,2).'</b>') + array_map('bold_format',$types_totals['']));
	unset($LO_types[0]);
	$LO_columns = array('TYPE'=>'Type','DISCOUNT'=>'Discount') + $users_columns + $types_columns + array('TOTAL'=>'Total');
}
else
{
	...
	foreach($types_totals[''] as $key=>$value)
		if($value == 0)
			unset($types_columns[$key]);
	$LO_types[] = array(array('TYPE'=>'<b>Totals</b>','ELLIGIBLE'=>'<b>'.number_format($users_totals['']['ELLIGIBLE'],1).'</b>','DAYS_POSSIBLE'=>'<b>'.number_format($users_totals['']['DAYS']/$users_totals['']['ELLIGIBLE'],1).'</b>','TOTAL_ELLIGIBLE'=>'<b>'.$users_totals['']['DAYS'].'</b>','PARTICIPATED'=>'<b>'.$users_totals['']['PARTICIPATED'].'</b>') + array_map('bold',$types_totals['']));
	unset($LO_types[0]);
	$LO_columns = array('TYPE'=>'Type','DISCOUNT'=>'Discount') + $users_columns + $types_columns;
}
...
{	global $THIS_RET,$types,$types_columns,$types_totals;
	...
	{
		$types_columns += array($value=>'<FONT color=red>'.$value.'</FONT>');
