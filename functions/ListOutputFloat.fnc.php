...
function ListOutputFloat($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false)
{
	...
	if($_LIST['output']!=false)
	{
		...
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			...
			// WIDTH = 100%
			echo '<TABLE class=grid cellspacing=0 cellpadding=1 >';
			...
			/*
				...
				*/
				...
				echo '<TR><TD width=100% align=right><DIV id=LOx'.(count($column_names)+(($result_count!=0 && $cols && !isset($_REQUEST['_CENTRE_PDF']))?1:0)+(($remove && !isset($_REQUEST['_CENTRE_PDF']))?1:0)).' style="width:0; position: relative; height:0;"></DIV></TD></TR>';
			// END SEARCH BOX ----
			echo '<TR><TD>';
