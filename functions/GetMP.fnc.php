...
{	global $_CENTRE;
	...
	if(!$_CENTRE['GetMP'])
	{
		$_CENTRE['GetMP'] = DBGet(DBQuery("SELECT MARKING_PERIOD_ID,TITLE,POST_START_DATE,POST_END_DATE,'SCHOOL_QUARTERS'        AS `TABLE`,SORT_ORDER,SHORT_NAME,START_DATE,END_DATE,DOES_GRADES,DOES_EXAM,DOES_COMMENTS FROM SCHOOL_QUARTERS         WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'
...
{	global $_CENTRE;
	...
	if(!$_CENTRE['GetCurrentMP'][$date][$mp])
	 	$_CENTRE['GetCurrentMP'][$date][$mp] = DBGet(DBQuery("SELECT MARKING_PERIOD_ID FROM $table WHERE '$date' BETWEEN START_DATE AND END_DATE AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'"));
